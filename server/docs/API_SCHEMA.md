...
- **Current (demo):** Lightweight agent-identity header — `X-Agent-Id` is **required** on all `/api/agent/*` routes. It is logged alongside `requestId` in every audit entry (`server/src/lib/auditLog.js`, `server/src/routes/agent.js` middleware) for attribution. **This is NOT cryptographic auth** — it is *identity attribution for audit purposes* only; anyone can set the header. When `AGENT_SHARED_SECRET` is set (single demo shared secret via env var, `server/src/lib/agentAuth.js`), the same routes also require `X-Agent-Timestamp` + `X-Agent-Signature` — HMAC-SHA256 over canonical `METHOD\nPATH\nX-Agent-Id\nBODY_JSON\nX-Agent-Timestamp` verified with `crypto.timingSafeEqual` and a 5-minute replay window (rejects >5m old or future timestamps). This proves the caller holds the shared secret and the request wasn't replayed, but it is **still not per-agent identity** — for hackathon scope a single demo secret is fine; **production would use per-agent keys issued at registration (e.g. per-agent HMAC secrets or asymmetric mTLS/OAuth client credentials), not one shared secret** — noted as a known limitation, not implemented here. `GET /api/catalog`, `POST /api/recommend`, `POST /api/checkout/*`, and `GET /api/audit` remain open for local demo. Checkout already gates on `userApproval:true` — under a protocol like ACP/AP2/x402 this would be replaced by a signed payment authorization token (see `ARCHITECTURE.md` > Agent-to-Agent Commerce Protocols).
- **What would change for production / protocol-native integration:** Replace single `AGENT_SHARED_SECRET` with per-agent key issuance at registration (or mTLS/OAuth client credentials), verify per-agent `X-Agent-Signature` via `verifyAgentSignature` before `agent.js` logic, and enforce per-agent ACL. Bind `agentId` to the verified per-agent identity before logging.
...
- **Current:** Idempotency is implemented for `POST /api/checkout/create-order`, `POST /api/agent/draft-order`, and `POST /api/merchant/growth-execute` via optional `Idempotency-Key` header (or reuse of `X-Request-Id` / `Request-Id` if already flowing). If a request with the same key arrives again on the same route, the original `200` response (same `orderId`/`razorpayOrder.id` or same `measured.orders`) is returned instead of creating a duplicate Razorpay order. Mappings are held in-memory (`server/src/lib/idempotency.js`) with a 24h TTL; no DB. Retrying without a key still creates a new order (visible via distinct `requestId`s). See `server/src/lib/idempotency.js` and `server/src/routes/checkout.js` / `server/src/routes/agent.js` / `server/src/routes/merchant.js`.
